...
if(not doCreateItemEx) then
	function doCreateItemEx(itemid, count)
		if(Game and type(Game) == "table" and Game.createItem and type(Game.createItem) == "function") then
			...
			print("[ERROR - gesior-shop-system] Error code: 1. Please contact Christianlb at Discord: Berzekz#8739")
		end
	end
end
				...
				if(not item_doesnt_exist) then
					...
					if(full_weight <= free_cap) then
						...
						if(received_item == RETURNVALUE_NOERROR) then
							doPlayerSendTextMessage(cid, messageType, "You received >> ".. add_item_name .." << from Tibia Nostalgia Store.")
							...
							doPlayerSendTextMessage(cid, messageType, '>> '.. add_item_name ..' << from Tibia Nostalgia Store is waiting for you. Please make place for this item in your backpack/hands and wait about '.. interval ..' seconds to get it.')
						end
					else
						doPlayerSendTextMessage(cid, messageType, '>> '.. add_item_name ..' << from Tibia Nostalgia Store is waiting for you. It weight is '.. full_weight ..' oz., you have only '.. free_cap ..' oz. free capacity. Put some items in depot and wait about '.. interval ..' seconds to get it.')
					end
				end
