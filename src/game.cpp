...
bool Game::loadExperienceStages()
{
    ...
    if (strcasecmp(stageNode.name(), "config") == 0) {
        ...
        uint32_t minLevel, maxLevel;  // Declare minLevel and maxLevel
        float multiplier;

        // Extract values from XML attributes
        minLevel = stageNode.attribute("minlevel").as_uint();
        maxLevel = stageNode.attribute("maxlevel").as_uint();
        multiplier = stageNode.attribute("multiplier").as_float(); // Make sure "multiplier" matches your XML attribute name
